...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/contacts/{id}", get(get_by_id).put(update).delete(remove))
}
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Contact details", body = serde_json::Value),
...
async fn get_by_id(
    ...
    let item = contact_service::get_by_id(&state.pool, id).await?;
