...
fn fmt_phone(rng: &mut impl Rng) -> String {
	format!("{} {:03}-{:02}-{:02}", pick(rng, PHONE_CODES), rng.random_range(100..999u32), rng.random_range(10..99u32), rng.random_range(10..99u32))
}

fn fmt_life_number(rng: &mut impl Rng) -> String {
	format!("RU{}{:012}", rng.random_range(0..999u32), rng.random_range(0u64..999999999999u64))
}
...
pub async fn seed_locations(pool: &PgPool) -> Vec<i32> {
	...
	for i in 0..LOCATIONS.len() {
		...
		.bind(LOCATIONS[i])
		.bind(LOCATION_TYPES.get(i).map(|_| LOCATION_TYPES[i.min(LOCATION_TYPES.len() - 1)]))
		.fetch_one(pool)
		.await
		.unwrap();
		ids.push(row.0);
...
pub async fn seed_contacts(pool: &PgPool) {
	...
	for i in 0..count {
		...
		let company = if rng.random_bool(0.6) { pick(&mut rng, COMPANIES) } else { "" };
		let phone = fmt_phone(&mut rng);
		let email = format!("{}.{}@farm{}.ru", first.to_lowercase(), last.to_lowercase(), i % 5 + 1);
...
pub async fn seed_feed_types(pool: &PgPool) -> Vec<i32> {
	let mut ids = Vec::new();
	let dry_matters = [85.0, 45.0, 30.0, 88.0, 89.0, 90.0, 95.0, 42.0, 86.0, 87.0, 92.0, 55.0];
	let prices = [5.0, 4.5, 3.5, 12.0, 14.0, 11.0, 18.0, 4.8, 2.5, 10.0, 8.0, 3.8];
...
pub async fn seed_animals(pool: &PgPool, config: &SeedConfig) -> Vec<AnimalInfo> {
	...
	for i in 0..total {
		...
		let age_days = normal_range(&mut rng, (age_min + age_max) / 2.0 * 365.0, 180.0, age_min * 365.0, age_max * 365.0) as i64;
	...
	for animal in animals {
		...
		for (j, code) in picks.iter().enumerate() {
			let pct = if j == picks.len() - 1 {
				...
				let p = normal_range(&mut rng, remaining / (picks.len() - j) as f64, 10.0, 5.0, remaining - 5.0 * (picks.len() - j - 1) as f64);
				remaining -= p;
				p
			};
	...
	for female in &females {
		...
		loop {
			...
			for _ in 0..num_calves {
				let calf_gender = if rng.random_bool(0.5) { "male" } else { "female" };
				let calf_ln = fmt_life_number(&mut rng);
				let birth_remark = if rng.random_bool(0.9) { "normal" } else { pick(&mut rng, &["abnormal_calf", "alive_premature_born", "abortion"]) };
				let weight = normal_range(&mut rng, 38.0, 5.0, 25.0, 55.0);
				let born_dead = rng.random_bool(0.03);
				let keep = if born_dead { false } else { rng.random_bool(0.85) };
				let calf_name = if calf_gender == "female" && !born_dead && rng.random_bool(0.7) {
					format!("'{}'", pick(&mut rng, HEIFER_NAMES).replace('\'', "''"))
				} else {
					"NULL".to_string()
				};
				let calf_gender_sql = if born_dead && *birth_remark == *"abortion" { "'male'".to_string() } else { format!("'{}'", calf_gender) };
			...
			for ai in 0..num_ai {
				...
				if !got_pregnant {
					let conception_chance = if ai == 0 { 0.45 } else if ai == 1 { 0.35 } else { 0.25 };
					if rng.random_bool(conception_chance) {
						got_pregnant = true;
						pregnant_at_day = ai_day;
						let preg_date = ai_date + Duration::days(normal_range(&mut rng, 30.0, 5.0, 25.0, 40.0) as i64);
						if preg_date <= today {
			...
			if got_pregnant && pregnant_at_day > 0 {
				...
				lactations.push(LactationInfo {
					...
					calving_date,
					lac_number: lac,
					dry_off_date: if dry_off <= today { Some(dry_off) } else { None },
					next_calving_date: Some(next_calving),
				});
		...
		for cd in &calving_dates {
			let remarks = if rng.random_bool(0.85) { "NULL".to_string() } else { format!("'{}'", pick(&mut rng, &["лёгкие", "нормальные", "тяжёлые", "с ветеринарной помощью"])) };
			calving_batch.push(format!(
				"({}, '{}', {}, {})",
				female.id, cd, calving_dates.iter().position(|d| d == cd).unwrap() + 1, remarks,
			));
		}
		...
		while date <= effective_end {
			...
			if milk > 0.5 {
				...
				for v in 0..num_visits {
					...
					let visit_milk = (milk / num_visits as f64) * normal_range(&mut rng, 1.0, 0.15, 0.5, 1.5);
					let duration = (visit_milk * 60.0 / normal_range(&mut rng, 2.0, 0.5, 1.0, 4.0)) as i32;
				...
				if day_of_year % 7 == 0 {
					...
					let scc = (normal_range(&mut rng, base_scc, 40000.0, 30000.0, 500000.0) / 1000.0).round() as i32 * 1000;
					...
					let refusals = if rng.random_bool(0.05) { rng.random_range(1..2) } else { 0 };
...
pub async fn seed_feed(pool: &PgPool, lactations: &[LactationInfo], config: &SeedConfig) {
	...
	for lact in lactations {
		...
		while date <= effective_end {
			...
			let feed_total = (milk_est * normal_range(&mut rng, 0.38, 0.05, 0.25, 0.55) + normal_range(&mut rng, 3.0, 1.0, 1.0, 6.0)) * 10.0;

			if feed_total > 5.0 {
				let feed_number = rng.random_range(1..4);
				day_batch.push(format!(
					"({}, '{}', {}, {})",
					lact.animal_id, date, feed_number, (feed_total).round() / 10.0,
				));
				total_days += 1;
				...
				for fv in 0..num_feed_visits {
					...
					let duration = (fv_amount * 60.0 / normal_range(&mut rng, 2.5, 0.5, 1.0, 5.0)) as i32;
					visit_batch.push(format!(
						"({}, '{} {:02}:{:02}:00+03', {}, {}, {})",
						lact.animal_id,
						date,
	...
	for lact in lactations {
		...
		while date <= effective_end {
			...
			for _ in 0..num_readings {
				...
				let minute = rng.random_range(0..60);
				let act_val = (base_activity * normal_range(&mut rng, 1.0, 0.15, 0.5, 1.8)).round() as i32;
...
pub async fn seed_bulk_tank(pool: &PgPool, config: &SeedConfig) {
	...
	while date <= today {
		...
		let scc = (normal_range(&mut rng, 180000.0, 40000.0, 80000.0, 350000.0) / 1000.0).round() as i32 * 1000;
	...
	for animal in animals {
		if !animal.active {
			...
			if transfer_date <= today {
				...
				batch.push(format!(
					...
					"выбытие",
					animal.location.as_deref().unwrap_or("Коровник 1 (основной)"),
					reason,
				));
				count += 1;
			}
		...
		if rng.random_bool(0.3) {
			...
			if transfer_date <= today {
				...
				let from_loc = animal.location.as_deref().unwrap_or("Коровник 1 (основной)");
...
pub async fn seed_sync_log(pool: &PgPool) {
	let entities = ["animals", "calvings", "inseminations", "milk_productions", "feed_data", "health_data", "grazing", "bulk_tank"];
	let mut batch = Vec::new();
	for entity in &entities {
		batch.push(format!("('{}', '{}', 'completed', {})", entity, chrono::Utc::now(), 100 + GLOBAL_RNG.lock().unwrap().random_range(0..1000)));
	}
	let sql = format!(
