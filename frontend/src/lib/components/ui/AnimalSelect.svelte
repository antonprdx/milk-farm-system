			...
			{:else}
				{#each results as animal, i (animal.id)}
					<li>
						<button
							type="button"
							class="w-full text-left px-3 py-2 text-sm text-slate-800 dark:text-slate-200 hover:bg-blue-50 dark:hover:bg-slate-800 transition-colors {i === highlightIndex ? 'bg-blue-50 dark:bg-slate-800' : ''}"
