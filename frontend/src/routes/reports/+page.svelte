<script lang="ts">
	...
	let thCls = 'px-3 py-2 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider';
	...
	let badgeRed = 'px-1.5 py-0.5 text-xs rounded bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400';
	let badgeYellow = 'px-1.5 py-0.5 text-xs rounded bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400';
	let badgeGreen = 'px-1.5 py-0.5 text-xs rounded bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400';
...
{#if activeTab !== 'udder-work' && activeTab !== 'udder-analyze' && activeTab !== 'calendar' && activeTab !== 'health-act' && activeTab !== 'efficiency' && activeTab !== 'lactation' && activeTab !== 'health-task' && activeTab !== 'pregnancy' && activeTab !== 'transition'}	<FilterBar bind:fromDate bind:tillDate showAnimal={false} onsearch={load} />
{/if}
...
{:else}
	...
	{#if activeTab === 'summary'}
		<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
			<div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-6 space-y-4">
				<h2 class="text-lg font-semibold text-slate-700 dark:text-slate-300">Молоко</h2>
				<div class="float-right flex gap-1">
					<a href={getExportUrl('milk', fromDate || undefined, tillDate || undefined, 'pdf')} class="text-xs px-2 py-1 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded hover:bg-red-100 dark:hover:bg-red-900/50 transition-colors">PDF</a>
					<a href={getExportUrl('milk', fromDate || undefined, tillDate || undefined)} class="text-xs px-2 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors">CSV</a>
				</div>
				{#if milk}
					<div class="space-y-2">
						<div class="flex justify-between text-sm"><span class="text-slate-500 dark:text-slate-400">Общий надой</span><span class="font-medium">{fmtNum(milk.total_milk)} л</span></div>
						<div class="flex justify-between text-sm"><span class="text-slate-500 dark:text-slate-400">Записей</span><span class="font-medium">{milk.count_days}</span></div>
						<div class="flex justify-between text-sm"><span class="text-slate-500 dark:text-slate-400">Среднее на животное</span><span class="font-medium">{fmtNum(milk.avg_per_animal)} л</span></div>
					</div>
				{/if}
			</div>
			<div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-6 space-y-4">
				...
				<div class="float-right flex gap-1">
					<a href={getExportUrl('reproduction', fromDate || undefined, tillDate || undefined, 'pdf')} class="text-xs px-2 py-1 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded hover:bg-red-100 dark:hover:bg-red-900/50 transition-colors">PDF</a>
					<a href={getExportUrl('reproduction', fromDate || undefined, tillDate || undefined)} class="text-xs px-2 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors">CSV</a>
				</div>
				{#if repro}
					<div class="space-y-2">
						<div class="flex justify-between text-sm"><span class="text-slate-500 dark:text-slate-400">Отёлы</span><span class="font-medium">{repro.total_calvings}</span></div>
						<div class="flex justify-between text-sm"><span class="text-slate-500 dark:text-slate-400">Осеменения</span><span class="font-medium">{repro.total_inseminations}</span></div>
						<div class="flex justify-between text-sm"><span class="text-slate-500 dark:text-slate-400">Стельности</span><span class="font-medium">{repro.total_pregnancies}</span></div>
						<div class="flex justify-between text-sm"><span class="text-slate-500 dark:text-slate-400">Охота</span><span class="font-medium">{repro.total_heats}</span></div>
						<div class="flex justify-between text-sm"><span class="text-slate-500 dark:text-slate-400">Запуски</span><span class="font-medium">{repro.total_dry_offs}</span></div>
					</div>
				{/if}
			</div>
			<div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-6 space-y-4">
				...
				<div class="float-right flex gap-1">
					<a href={getExportUrl('feed', fromDate || undefined, tillDate || undefined, 'pdf')} class="text-xs px-2 py-1 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded hover:bg-red-100 dark:hover:bg-red-900/50 transition-colors">PDF</a>
					<a href={getExportUrl('feed', fromDate || undefined, tillDate || undefined)} class="text-xs px-2 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors">CSV</a>
				</div>
				{#if feed}
					<div class="space-y-2">
						<div class="flex justify-between text-sm"><span class="text-slate-500 dark:text-slate-400">Общий корм</span><span class="font-medium">{fmtNum(feed.total_feed_kg)} кг</span></div>
						<div class="flex justify-between text-sm"><span class="text-slate-500 dark:text-slate-400">Визитов</span><span class="font-medium">{feed.total_visits}</span></div>
					</div>
				{/if}
			</div>
		</div>
	...
	{:else if activeTab === 'rest-feed' && restFeedData}
		{#if restFeedData.total_rest_feed_pct != null}
			<div class="mb-4 bg-white dark:bg-slate-800 rounded-lg border border-slate-100 dark:border-slate-700 p-3">
				...
				<span class="font-semibold {restFeedData.total_rest_feed_pct > 5 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}">
		...
		<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
			<table class={tblCls}>
				<thead class="bg-slate-50 dark:bg-slate-900/50">
					<tr><th class={thCls}>Животное</th><th class={thCls}>Дата</th><th class={thCls}>№ Корма</th>
				...
				<tbody class="divide-y divide-slate-200 dark:divide-slate-700">
					{#each restFeedData.rows as row (row.animal_id + '-' + row.feed_date + '-' + row.feed_number)}
						<tr><td class={tdCls}>{row.animal_name ?? row.animal_id}</td><td class={tdCls}>{row.feed_date}</td>
							...
							<td class={tdCls}><span class={row.rest_feed_pct && row.rest_feed_pct > 10 ? 'text-red-600 dark:text-red-400 font-medium' : ''}>{fmtNum(row.rest_feed_pct)}%</span></td></tr>
					{/each}
				</tbody>
			</table>
		</div>
	...
	{:else if activeTab === 'robot'}
		<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
			<table class={tblCls}>
				<thead class="bg-slate-50 dark:bg-slate-900/50">
					<tr><th class={thCls}>Робот</th><th class={thCls}>Дата</th><th class={thCls}>Ср. скорость</th>
						<th class={thCls}>Макс. скорость</th><th class={thCls}>Доек</th>
						<th class={thCls}>LF время</th><th class={thCls}>LR время</th><th class={thCls}>RF время</th><th class={thCls}>RR время</th>
				...
				<tbody class="divide-y divide-slate-200 dark:divide-slate-700">
					{#each robotData as row (row.device_address + '-' + row.date)}
						<tr><td class={tdCls}>{row.device_address ?? '—'}</td><td class={tdCls}>{row.date}</td>
							...
							<td class={tdCls}>{fmtNum(row.avg_lf_milk_time, 0)}</td><td class={tdCls}>{fmtNum(row.avg_lr_milk_time, 0)}</td>
							<td class={tdCls}>{fmtNum(row.avg_rf_milk_time, 0)}</td><td class={tdCls}>{fmtNum(row.avg_rr_milk_time, 0)}</td>
							<td class={tdCls}><span class={row.avg_lf_dead_milk_time && row.avg_lf_dead_milk_time > 60 ? 'text-red-600 dark:text-red-400 font-medium' : ''}>{fmtNum(row.avg_lf_dead_milk_time, 0)}</span></td>
							<td class={tdCls}><span class={row.avg_lr_dead_milk_time && row.avg_lr_dead_milk_time > 60 ? 'text-red-600 dark:text-red-400 font-medium' : ''}>{fmtNum(row.avg_lr_dead_milk_time, 0)}</span></td>
							<td class={tdCls}><span class={row.avg_rf_dead_milk_time && row.avg_rf_dead_milk_time > 60 ? 'text-red-600 dark:text-red-400 font-medium' : ''}>{fmtNum(row.avg_rf_dead_milk_time, 0)}</span></td>
							<td class={tdCls}><span class={row.avg_rr_dead_milk_time && row.avg_rr_dead_milk_time > 60 ? 'text-red-600 dark:text-red-400 font-medium' : ''}>{fmtNum(row.avg_rr_dead_milk_time, 0)}</span></td></tr>
					{/each}
				</tbody>
			</table>
		</div>
	...
	{:else if activeTab === 'failed'}
		<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
			<table class={tblCls}>
				<thead class="bg-slate-50 dark:bg-slate-900/50">
					<tr><th class={thCls}>Животное</th><th class={thCls}>Время</th><th class={thCls}>Робот</th>
				...
				<tbody class="divide-y divide-slate-200 dark:divide-slate-700">
					{#each failedData as row (row.animal_id + '-' + row.visit_datetime)}
						<tr><td class={tdCls}>{row.animal_name ?? row.animal_id}</td><td class={tdCls}>{new Date(row.visit_datetime).toLocaleString('ru-RU')}</td>
							...
							<td class={tdCls}><span class={row.rr_conductivity && row.rr_conductivity > 83 ? 'text-red-600 dark:text-red-400 font-medium' : ''}>{row.rr_conductivity ?? '—'}</span></td></tr>
					{/each}
				</tbody>
			</table>
		</div>
	...
	{:else if (activeTab === 'udder-work' || activeTab === 'udder-analyze')}
		...
		<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
			<table class={tblCls}>
				<thead class="bg-slate-50 dark:bg-slate-900/50">
					<tr><th class={thCls}>Животное</th><th class={thCls}>Время</th>
	...
	{:else if activeTab === 'milk-time'}
		<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
			<table class={tblCls}>
				<thead class="bg-slate-50 dark:bg-slate-900/50">
					<tr><th class={thCls}>Дата</th><th class={thCls}>Коров</th><th class={thCls}>Надой (л)</th>
						<th class={thCls}>Средн./корова</th><th class={thCls}>Доек</th><th class={thCls}>Отказов</th>
				...
				<tbody class="divide-y divide-slate-200 dark:divide-slate-700">
					{#each milkTime as row (row.date)}
						<tr><td class={tdCls}>{row.date}</td><td class={tdCls}>{row.cow_count}</td>
							...
							<td class={tdCls}>{fmtNum(row.total_feed)}</td><td class={tdCls}>{row.total_rest_feed ?? '—'}</td></tr>
					{/each}
				</tbody>
			</table>
		</div>
	...
	{:else if activeTab === 'visit'}
		<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
			<table class={tblCls}>
				<thead class="bg-slate-50 dark:bg-slate-900/50">
					<tr><th class={thCls}>Животное</th><th class={thCls}>Доек</th><th class={thCls}>Отказов</th>
				...
				<tbody class="divide-y divide-slate-200 dark:divide-slate-700">
					{#each visitData as row (row.animal_id)}
						<tr><td class={tdCls}>{row.animal_name ?? row.animal_id}</td><td class={tdCls}>{row.total_milkings}</td>
							...
							<td class={tdCls}>{row.last_visit ? new Date(row.last_visit).toLocaleString('ru-RU') : '—'}</td></tr>
					{/each}
				</tbody>
			</table>
		</div>
		...
		<div class="space-y-6">
			<div>
				...
				<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
					<table class={tblCls}>
						<thead class="bg-slate-50 dark:bg-slate-900/50">
							<tr><th class={thCls}>Животное</th><th class={thCls}>Лактация</th><th class={thCls}>Последняя инсем.</th>
						...
						<tbody class="divide-y divide-slate-200 dark:divide-slate-700">
							{#each calendarData.expected_calvings as row (row.animal_id)}
								<tr><td class={tdCls}>{row.animal_name ?? row.animal_id}</td><td class={tdCls}>{row.lac_number ?? '—'}</td>
									...
									<td class={tdCls}>{row.sire_code ?? '—'}</td><td class={tdCls}>{row.days_pregnant ?? '—'}</td></tr>
							{/each}
						</tbody>
					</table>
				</div>
			...
			<div>
				...
				<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
					<table class={tblCls}>
						<thead class="bg-slate-50 dark:bg-slate-900/50">
							<tr><th class={thCls}>Животное</th><th class={thCls}>Ожид. отёл</th><th class={thCls}>Реком. запуск</th>
			...
			<div>
				...
				<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
					<table class={tblCls}>
						<thead class="bg-slate-50 dark:bg-slate-900/50">
							<tr><th class={thCls}>Животное</th><th class={thCls}>Посл. охота</th><th class={thCls}>Ожид. охота</th>
			...
			<div>
				...
				<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
					<table class={tblCls}>
						<thead class="bg-slate-50 dark:bg-slate-900/50">
							<tr><th class={thCls}>Животное</th><th class={thCls}>Дата инсем.</th><th class={thCls}>Бык</th>
	...
	{:else if activeTab === 'health-act'}
		<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
			<table class={tblCls}>
				<thead class="bg-slate-50 dark:bg-slate-900/50">
					<tr><th class={thCls}>Животное</th><th class={thCls}>Health Index</th><th class={thCls}>Откл. активн.</th>
				...
				<tbody class="divide-y divide-slate-200 dark:divide-slate-700">
					{#each healthAct as row (row.animal_id)}
						<tr><td class={tdCls}>{row.animal_name ?? row.animal_id}</td>
							...
							<td class={tdCls}>{fmtNum(row.latest_milk)}</td><td class={tdCls}>{fmtNum(row.avg_milk_7d)}</td>
	...
	{:else if activeTab === 'efficiency'}
		<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
			<table class={tblCls}>
				<thead class="bg-slate-50 dark:bg-slate-900/50">
					<tr><th class={thCls}>Животное</th><th class={thCls}>Молоко/мин/нед</th><th class={thCls}>Ср. скорость</th>
		...
		{#each lactationData as lac (lac.lac_number)}
			<div class="mb-6">
				...
				<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
					<table class={tblCls}>
						<thead class="bg-slate-50 dark:bg-slate-900/50">
							<tr><th class={thCls}>DIM</th><th class={thCls}>Средн. надой</th><th class={thCls}>Визитов</th>
						...
						<tbody class="divide-y divide-slate-200 dark:divide-slate-700">
							{#each lac.points as pt (pt.dim)}
								<tr><td class={tdCls}>{pt.dim}</td><td class={tdCls}>{fmtNum(pt.avg_milk)}</td>
									...
									<td class={tdCls}>{fmtNum(pt.avg_protein)}</td><td class={tdCls}>{pt.cow_count}</td></tr>
							{/each}
						</tbody>
					</table>
				</div>
	...
	{:else if activeTab === 'feed-type' && feedTypeData}
		...
		<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
			<table class={tblCls}>
				<thead class="bg-slate-50 dark:bg-slate-900/50">
					<tr><th class={thCls}>Дата</th><th class={thCls}>Тип</th><th class={thCls}>Название</th>
	...
	{:else if activeTab === 'feed-cow'}
		<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
			<table class={tblCls}>
				<thead class="bg-slate-50 dark:bg-slate-900/50">
					<tr><th class={thCls}>Дата</th><th class={thCls}>Коров</th><th class={thCls}>Корм/корова</th>
				...
				<tbody class="divide-y divide-slate-200 dark:divide-slate-700">
					{#each feedCowData as row (row.date)}
						<tr><td class={tdCls}>{row.date}</td><td class={tdCls}>{row.animal_count}</td>
							...
							<td class={tdCls}>{fmtNum(row.avg_rumination_minutes, 0)}</td><td class={tdCls}>{fmtNum(row.avg_day_production)}</td>
							<td class={tdCls}>{fmtNum(row.avg_lactation_days, 0)}</td><td class={tdCls}>{fmtNum(row.feed_efficiency)}</td></tr>
					{/each}
				</tbody>
			</table>
		</div>
	...
	{:else if activeTab === 'health-task' && healthTaskData}
		<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
			<table class={tblCls}>
				<thead class="bg-slate-50 dark:bg-slate-900/50">
					<tr><th class={thCls}>Животное</th><th class={thCls}>Sick Chance</th><th class={thCls}>Статус</th>
	...
	{:else if activeTab === 'pregnancy' && pregnancyData}
		<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
			<table class={tblCls}>
				<thead class="bg-slate-50 dark:bg-slate-900/50">
					<tr><th class={thCls}>Конец периода</th><th class={thCls}>Пригодных</th><th class={thCls}>Осеменено</th>
				...
				<tbody class="divide-y divide-slate-200 dark:divide-slate-700">
					{#each pregnancyData.periods as row (row.end_date)}
						<tr><td class={tdCls}>{row.end_date}</td><td class={tdCls}>{row.eligible}</td>
							...
							<td class={tdCls}><span class={row.pregnancy_rate && row.pregnancy_rate < 25 ? 'text-red-600 dark:text-red-400 font-medium' : 'font-medium'}>{fmtNum(row.pregnancy_rate)}%</span></td></tr>
					{/each}
				</tbody>
			</table>
		</div>
	...
	{:else if activeTab === 'transition' && transitionData}
		<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
			<table class={tblCls}>
				<thead class="bg-slate-50 dark:bg-slate-900/50">
					<tr><th class={thCls}>Животное</th><th class={thCls}>День (относ.)</th><th class={thCls}>Надой 24ч</th>
				...
				<tbody class="divide-y divide-slate-200 dark:divide-slate-700">
					{#each transitionData.rows as row (row.animal_id)}
						<tr><td class={tdCls}>{row.animal_name ?? row.animal_id}</td>
							...
							<td class={tdCls}><span class={row.rumination_3day_diff && row.rumination_3day_diff < -60 ? 'text-red-600 dark:text-red-400 font-medium' : ''}>{row.rumination_3day_diff ?? '—'}</span></td>
							<td class={tdCls}>{row.rumination_minutes ?? '—'}</td>
							<td class={tdCls}>{fmtNum(row.feed_total)}</td><td class={tdCls}>{row.feed_rest ?? '—'}</td>
