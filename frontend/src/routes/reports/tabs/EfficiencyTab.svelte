...
<div class="overflow-x-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
	<table class={tblCls}>
		<thead class="bg-slate-50 dark:bg-slate-900/50">
			<tr>
				<th class={thCls}>Животное</th><th class={thCls}>Молоко/мин/нед</th><th class={thHidden}>Ср. скорость</th>
				<th class={thHidden}>Время обработки</th><th class={thHidden}>Время доения</th><th class={thCls}>Доек/7д</th>
				<th class={thCls}>Надой/7д</th><th class={thHidden}>Средн./доение</th>
			</tr>
		</thead>
		<tbody class="divide-y divide-slate-200 dark:divide-slate-700">
			{#each rows as row (row.animal_id)}
				<tr>
					...
					<td class={tdHidden}>{fmtNum(row.avg_milking_time)} мин</td>
					<td class={tdCls}>{row.milkings_7d}</td>
					<td class={tdCls}>{fmtNum(row.total_milk_7d)}</td>
					<td class={tdHidden}>{fmtNum(row.avg_milk_per_milking)}</td>
				</tr>
			{/each}
		</tbody>
	</table>
