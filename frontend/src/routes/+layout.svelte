...
{/if}
	...
	<div class="flex h-screen bg-slate-100 dark:bg-slate-900">
		{#if mobileMenuOpen}
			<div
				class="fixed inset-0 bg-black/50 z-40 md:hidden {mobileMenuClosing ? 'mobile-overlay-exit' : 'mobile-overlay-enter'}"
		...
		{#if mobileMenuOpen}
			<div class="fixed inset-y-0 left-0 z-50 md:hidden {mobileMenuClosing ? 'mobile-sidebar-exit' : 'mobile-sidebar-enter'}">
				<Sidebar collapsed={false} {isAdmin} onclose={closeMobileMenu} />
			</div>
		{/if}

		<div class="flex-1 flex flex-col min-w-0">
			<header class="md:hidden flex items-center gap-3 px-4 py-3 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
				<button
			...
			<main class="flex-1 overflow-auto p-4 md:p-6 md:transition-all md:duration-300 main-with-sidebar" style="--sidebar-w: {sidebarCollapsed ? '4rem' : '14rem'}">
				{#if mustChangePassword && !pathname.startsWith('/settings')}
					<div class="mb-4 p-3 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
						<span class="text-sm text-amber-800 dark:text-amber-200">Необходимо сменить пароль для безопасной работы.</span>
						<button
